...
export const MobileLeadership = () => {
  ...
  const sliderTL = useRef<any>(null);

  const newLeadershipScene = (gsapCtx: any) => {
    gsapCtx = teamAnimationCtx.current = gsap.context((ctx) => {
      if (!teamSliderRef.current) return;

      // const xToPercent = width * -0.15
      const teamMemberCards = Array.from(teamSliderRef.current?.children || []);
      ...
      const goToNextCard = () => {
        if (currentActiveCard.current === teamMembersData.length - 1) {
          goToIndex(currentActiveCard.current, 0);
          return;
        }
        goToIndex(currentActiveCard.current, currentActiveCard.current + 1);
      };

      const goToPrevCard = () => {
        if (currentActiveCard.current === 0) {
          goToIndex(currentActiveCard.current, teamMembersData.length - 1);
          return;
        }
        goToIndex(currentActiveCard.current, currentActiveCard.current - 1);
      };

      const goToIndex = (prev: number, current: number) => {
        sliderTL.current.toIndex(current, {
          ease: "power2.out",
          duration: 0.7,
        });
      };

      sliderTL.current = horizontalSlider(teamMemberCards, {
        paused: true,
        speed: 5,
        draggable: true,
        onDragRelease: (closestIndex: number) => {
          currentActiveCard.current = closestIndex;
          updateProgressBar(closestIndex / (teamMembersData.length - 1));
          sliderTL.current.toIndex(closestIndex, {
            ease: "power2.out",
            duration: 0.5,
          });
        ...
        onChange: (element: any, index: number) => {
          ...
          updateProgressBar(index / (teamMembersData.length - 1));
        },
      });
  ...
  const goToProgress = (e: any) => {
    ...
    teamAnimationCtx.current.goToIndex(currentActiveCard.current, targetIndex);
  };
  ...
  const scrollToSlides = () => {
    smoothScroller.scrollTo(window.innerHeight, {
      duration: 2,
    });
  };
  ...
  return (
    <section
      className={`container ${styles.m_leadership_section}`}
      ...
      >
        ...
        <div className={styles.intro_slider} ref={IntroSliderRef}>
          ...
          <div className={styles.team_members_wrapper} ref={teamMembersWrapper}>
            ...
            <div className={styles.team_slider_scroller} ref={teamSliderRef}>
...
const MemberCard = ({
  ...
  return (
    <div className={styles.card_memeber_wrapper}>
      <div className={styles.memeber_card} ref={MemberCardRef}>
        ...
        <button
          className={styles.open_member_popup_mobile}
