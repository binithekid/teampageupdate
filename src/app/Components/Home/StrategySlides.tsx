...
const strategySlides = [
  {
    ...
    content: "Investors looking to enhance traditional infrastructure returns through a specialized, operationally intensive focus on mid-cap growth in the fast-growing digital infrastructure sector"
  },
  {
    supTitle: "Risk",
    title: "Enhanced risk <br/>mitigation",
    content: "Investors seeking to maintain traditional infrastructure risk profiles in the rapidly growing and evolving digital infrastructure sector, through an intensive industry and operational specialism, whilst maintaining a focus on the more traditional infrastructure assets and models within the sector"
  },
  {
    supTitle: "Sustainability and digital inclusion",
    title: "Sustainable and digital <br/>inclusion mandate",
    content: "Bringing innovative investment and operational strategies for higher levels of digital inclusivity and sustainability"
  },
]
...
export const StrategySlides = () => {
  ...
  const strategySlidesAnimation = (gsapCtx: any) => {
    ...
    gsapCtx = gsap.context(() => {
      ...
      if ( !strategySliderRef.current || !strategyHeadingContainer.current || !strategySliderNavRef.current ) return

      gsap.fromTo(strategyHeadingContainer.current.firstChild, {
        alpha: 0,
        yPercent: 50
      }, {
        alpha: 1,
      ...
      const slideItems = strategySliderRef.current.querySelectorAll(".strategy_slide_item")

      const firstSlideCounterSpan      = slideItems[0].querySelector(".strategy_counter")
      const firstSlideHeadingHeading   = slideItems[0].querySelector(".slide_heading_container h4")
      const firstSlideContentParagraph = slideItems[0].querySelector(".slide_content_container p")
      ...
      const goToLottieSlide = (prevSlideIdx: number, nextSlideIdx: number) => {
        ...
        lottieAnimationEl.current.getLottie()?.goToAndStop(0,true)
          ...
          lottieAnimationEl.current.getLottie()?.goToAndStop(secondSlideFrameEnd,true)
        else if ( nextSlideIdx === 2 ) 
          lottieAnimationEl.current.getLottie()?.goToAndStop(thirdSlideFrameEnd,true)
      ...
      const updateSlideProgressNav = (newProgress: number) => {
        ...
        let navItemProgress = (newProgress - (activeSlideCounter.current * ( progressPart ))) / progressPart
      ...
      Array.from(strategySliderNavRef.current.children).forEach( (strategySlideTab,idx) => {
        strategySlideTab.addEventListener("click", () => {
          if ( !strategySlidesSectionRef.current ) return

          const targetScroll = strategySlidesSectionRef.current.offsetTop + ( idx * window.innerHeight * 1.1 ) + 80
      ...
      const goToSlide = (prevSlide: number,nextSlide: number) => {
        ...
        const prevSlideCounterSpan      = slideItems[prevSlide].querySelector(".strategy_counter")
        const prevSlideHeadingHeading = slideItems[prevSlide].querySelector(".slide_heading_container h4")
        const prevSlideContentParagraph = slideItems[prevSlide].querySelector(".slide_content_container p")

        const nextSlideCounterSpan      = slideItems[nextSlide].querySelector(".strategy_counter")
        const nextSlideHeadingHeading = slideItems[nextSlide].querySelector(".slide_heading_container h4")
        const nextSlideContentParagraph = slideItems[nextSlide].querySelector(".slide_content_container p")
        ...
        gsap.fromTo([prevSlideHeadingHeading,prevSlideContentParagraph], {
          y: 0
        }, {
          y: "-150%",
          duration: 0.5,
        ...
        gsap.fromTo([nextSlideHeadingHeading,nextSlideContentParagraph], {
          y: "150%"
        }, {
          y: 0,
          duration: 0.5,
      ...
      ScrollTrigger.create({
        ...
        onUpdate: (e) => {
          ...
          if ( activeSlideCounter.current !== currentActiveSlide && currentActiveSlide <= slideItems.length - 1 ) {
            if ( currentActiveSlide > activeSlideCounter.current ) fillNavProgressBar(activeSlideCounter.current)
  ...
  return (
    <section className={`${styles.strategy_slides_section} container`} ref={strategySlidesSectionRef}>
      
      <div className={styles.strategy_container} >

        <div className={styles.heading_container} ref={strategyHeadingContainer} >
          <h6>Digital Gravity’s strategy is positioned towards</h6>
        </div>
...

          {
            deviceSize > 768 ?
              <dotlottie-player
                ...
                subframe={false}
              />
            : <Image src={MobileSlideImages[activeImageIdx]} alt="strategy image" fill />
          }
        ...
        <div className={styles.slides_right_part} >
          ...
          <div className={styles.strategy_slides_container} ref={strategySliderRef} >

            {
              strategySlides.map( ({ supTitle, title, content }, idx) =>
                <div className={`${styles.strategy_slide} strategy_slide_item`} key={title} >
                  
                  <span className={`${styles.strategy_counter} strategy_counter`} >{slideCounter(idx + 1)} <span></span> {supTitle}</span>

                  <div className={`${styles.slide_heading_container} slide_heading_container`} >
                    <h4 dangerouslySetInnerHTML={{ __html: title }}></h4>
                  </div>

                  <div className={`${styles.slide_content_container} slide_content_container`}>
